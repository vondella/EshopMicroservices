...
using Play.Trading.Activities;

namespace Play.Trading.StateMachines
{
    public class PurchaseStateMachine:MassTransitStateMachine<PurchaseState>
    {
        ...
        public void ConfigureEvents()
        {
            ...
            Event(() => GetPurchaseState, x => x.CorrelateById(context=>context.Message.Id));
            Event(() => InventoryItemsGranted,x=>x.CorrelateById(m=>m.Message.CorrelarionId));
            Event(() => GilDebited, x => x.CorrelateById(m => m.Message.CorrelationId));
        ...
        private void ConfigreAccepted()
        {
            During(Accepted,
                ...
                .Send(context=> new 
                DebitGil(context.Instance.UserId,
                ...
                .Then(context =>
                {
                    context.Instance.ErrorMessage = context.Message.Exceptions[0].Message;
                    context.Instance.LastUpdated = DateTimeOffset.UtcNow;
                }).TransitionTo(Faulted));
        }
        public void ConfigureItemGranted()
        {
            During(ItemsGranted,
                Ignore(PurchaseRequested),
                ...
                Then(context =>
              {
                ...
                When(DebitGilFaulted).Send(context =>
                new SubtractItems(context.Instance.UserId,
                context.Instance.ItemId,
                context.Instance.Quantity,
                context.Instance.CorrelationId))
                .Then(context =>
        ...
        public void ConfigureFaulted()
        {
            During(Faulted, Ignore(GilDebited), Ignore(PurchaseRequested), Ignore(InventoryItemsGranted));
        }
        public void ConfigureCompleted()
        {
            During(Completed, Ignore(PurchaseRequested),Ignore(InventoryItemsGranted),Ignore(GilDebited));
        }
    }
}
